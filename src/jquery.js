...
(function( window, undefined ) {
	...
	var
		...
		// 正确的使用 document 和 window 的属性（沙箱）
		...
		// [+-] 匹配[]里包含的任意字符，? 匹配0或1个
		// ?:pattern 匹配 pattern 但不获得结果 常与 “|” 一起用
		// \d = [0-9]  "*" 匹配前面的子表达式任意次数 "+" 匹配{1}
		...
		detach = function() {
			if ( document.addEventListener ) {
				// 当浏览器把DOM树构建完成后就开始触发了DOMContentLoaded事件
				document.removeEventListener( "DOMContentLoaded", completed, false );
				// load事件则要等包括图片这些加载完毕才会触发
	...
	jQuery.extend({
		...
		ready: function( wait ) {
			...
			if ( wait === true ? --jQuery.readyWait : jQuery.isReady ) { // 还在等待的话就 return
				return;
			}
	...
	*/
	(function( window, undefined ) {
		...
		// Sizzle 方法是 Sizzle 选择器包的主要入口，find 方法就是调用该方法获取匹配的节点
		// 改方法主要完成下列任务
		// 1、对于单一选择器，且是 ID、Tag、Class三种类型之一，则直接获取并返回结果
		// 2、对于支持 querySelectorAll 方法的浏览器，通过执行 querySelectorAll 方法并返回匹配的 DOM 元素
		// 3、除上之外则调用 select 方法获取并返回匹配的 DOM 元素

		// selector: 选择器字符串 
		// context: 执行匹配的最初的上下文（即 DOM 元素集合）。若 context 没有赋值。则取 document。
		// results 已匹配出的部分最终结果。若 result 没有赋值，则赋予空数组
		// seed 初始集合
		function Sizzle( selector, context, results, seed ) {
			var match, elem, m, nodeType,
				// QSA(querySelectorAll) vars
				i, groups, old, nid, newContext, newSelector;

			// preferredDoc = window.document
			// setDocument 方法完成一些初始化工作
			...
			// 若 selector 不是有效的字符串类型数据，则直接返回 results
			if ( !selector || typeof selector !== "string" ) {
				return results;
			}

			// 若 context 既不是 document (nodeType = 9)，也不是 element (nodeType = 1)，就返回空集合
			if ( (nodeType = context.nodeType) !== 1 && nodeType !== 9 ) {
				return [];
			}

			// 若当前过滤的石 HTML 文档，且没有设定 seed，则执行 if 体内的内容
				...
				if ( (match = rquickExpr.exec( selector )) ) {
					...
					if ( (m = match[1]) ) {
						...
						if ( nodeType === 9 ) {
							...
							// contains( context, elem ) 用来确定获取的 elem 是否是当前 context 对象的子对象
					...
					// 处理 class类型选择器，如：.class
					// 下面的判断条件分分别是：
					// m = match[3]) 有效的 class 类型选择器
					// support.getElementsByClassName 该选择器的 p 支持 getElementsByClassName
					// context.getElementsByClassName 当前上下文有 getElementsByClassName
         ...
         */
				// support = Sizzle.support = {}; line 1532
				if ( support.qsa && (!rbuggyQSA || !rbuggyQSA.test( selector )) ) {
					...
					if ( nodeType === 1 && context.nodeName.toLowerCase() !== "object" ) {
						groups = tokenize( selector ); // tokenize 2492
						...
						/*
						 * rsibling = new RegExp(whitespace + *[+~])
						 * rsibling 用于判定选择器是否存在兄弟关系符
						 * 若包含 +~ 符号，则取 context 的父节点作为当前节点
						*/
		...
		*/
		...
		// preferredDoc 上文定义的 = window.document
		// 在浏览器窗口中，document.defaultView 返回与文档关联的窗口对象，如果没有，则返回 null。
